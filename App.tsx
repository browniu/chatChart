...
const MainContent: React.FC = () => {
  ...
  const handleApplyPalette = (paletteKey: keyof typeof PALETTES) => {
    ...
    if (newConfig.series) {
        newConfig.series = newConfig.series.map((s, idx) => ({ ...s, color: colors[idx % colors.length] }));
  ...
  const handleHistorySelect = (item: HistoryItem) => {
    ...
    setEditableCode(JSON.stringify(item.config, null, 2));
  ...
  const handleClearHistory = () => {
    if(confirm(lang === 'zh' ? "确认清空所有历史记录？" : "Clear all history?")) setHistory([]);
  };
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-950 font-sans text-slate-800 dark:text-slate-100 overflow-hidden transition-colors">
      ...
      {isSidebarOpen && <div className="fixed inset-0 bg-black/50 z-10 lg:hidden" onClick={() => setSidebarOpen(false)} />}

      <div className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 ${isSidebarVisible ? 'lg:w-80' : 'lg:w-0'} transition-all duration-300 ease-in-out fixed lg:static z-20 h-full flex-shrink-0 overflow-hidden border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900`}>
        <div className="w-80 h-full"><HistorySidebar isOpen={true} history={history} onSelect={handleHistorySelect} onClear={handleClearHistory} lang={lang} /></div>
      </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden w-full">
        <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 p-4 flex justify-between items-center shadow-sm z-10 flex-shrink-0 transition-colors">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="lg:hidden p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg text-gray-600 dark:text-gray-300"><Menu size={20} /></button>
            <button onClick={() => setSidebarVisible(!isSidebarVisible)} className="hidden lg:block p-2 text-gray-500 hover:text-orange-600 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors">{isSidebarVisible ? <PanelLeftClose size={20} /> : <PanelLeftOpen size={20} />}</button>
            <div className="flex items-center gap-2">
               <div className="bg-orange-500 p-2 rounded-lg text-white shadow-lg shadow-orange-500/30"><Sparkles size={20} fill="white" /></div>
               <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-orange-500 to-amber-600 dark:from-orange-400 dark:to-amber-500">{t.appTitle}</h1>
            </div>
          </div>
          
          <div className="flex gap-2 items-center">
             <button onClick={() => setLang(lang === 'zh' ? 'en' : 'zh')} className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 transition-colors flex items-center gap-1 font-medium text-sm"><Languages size={18} /> {lang.toUpperCase()}</button>

             <div className="relative">
                <button onClick={() => setPaletteMenuOpen(!isPaletteMenuOpen)} className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 transition-colors flex items-center gap-2">
                  <Palette size={20} />
                ...
                {isPaletteMenuOpen && (
                  <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden z-50">
                    {Object.entries(PALETTES).map(([key, palette]) => (
                      <button key={key} onClick={() => handleApplyPalette(key as keyof typeof PALETTES)} className={`w-full text-left px-4 py-3 text-sm flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${selectedPalette === key ? 'text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-gray-700/50' : 'text-gray-700 dark:text-gray-300'}`}>
                         <div className="flex gap-1">{palette.colors.slice(0, 3).map(c => <div key={c} className="w-3 h-3 rounded-full" style={{ backgroundColor: c }} />)}</div>
                         {palette.name}
                      </button>
                    ))}
                  </div>
                )}
                {isPaletteMenuOpen && <div className="fixed inset-0 z-40" onClick={() => setPaletteMenuOpen(false)} />}
             </div>

             <div className="h-6 w-px bg-gray-200 dark:bg-gray-700 mx-1"></div>
             <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 transition-colors">{isDarkMode ? <Sun size={20} /> : <Moon size={20} />}</button>
             <button onClick={() => setIsSettingsOpen(true)} className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 transition-colors"><Settings size={20} /></button>
            ...
            {currentConfig && (
                 <div className="flex gap-1 ml-2">
                    ...
                    <button onClick={handleCopyImage} className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-orange-600 dark:hover:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/30 rounded-lg transition-colors border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex-1 flex flex-row overflow-hidden relative">
          ...
          >
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex-shrink-0 space-y-3">
               <div className="flex gap-2">
                 <button onClick={() => setPrompt(INITIAL_PROMPT)} className="text-xs flex items-center gap-1 px-2 py-1 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 rounded text-gray-700 dark:text-gray-300 transition-colors"><BarChart3 size={12}/> Data Chart</button>
                 <button onClick={() => setPrompt(FLOWCHART_PROMPT)} className="text-xs flex items-center gap-1 px-2 py-1 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 rounded text-gray-700 dark:text-gray-300 transition-colors"><Workflow size={12}/> Flowchart</button>
               </div>
               <div className="relative">
                  {selectedImage ? (
                    <div className="relative w-full h-32 bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-600 flex items-center justify-center group">
                        ...
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                           <button onClick={() => fileInputRef.current?.click()} className="p-2 bg-white/20 hover:bg-white/40 rounded-full text-white backdrop-blur-sm transition-colors"><Upload size={16} /></button>
                           <button onClick={() => setSelectedImage(null)} className="p-2 bg-red-500/80 hover:bg-red-600 rounded-full text-white backdrop-blur-sm transition-colors"><X size={16} /></button>
                        </div>
                    </div>
                  ) : (
                    <div onClick={() => fileInputRef.current?.click()} className="w-full h-16 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg flex items-center justify-center cursor-pointer hover:border-orange-500 dark:hover:border-orange-500 hover:bg-orange-50 dark:hover:bg-gray-700/50 transition-colors">
                        <span className="text-gray-400 text-sm flex items-center gap-2"><ImageIcon size={18} /> {t.upload.placeholder}</span>
                    </div>
                  )}
                  <input type="file" ref={fileInputRef} onChange={handleFileSelect} className="hidden" accept="image/*" />
               </div>
               <div className="relative">
                <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder={t.describePlaceholder} className="w-full h-24 p-3 pr-12 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none resize-none text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 text-sm shadow-sm transition-colors" />
                <div className="absolute top-[-40px] right-0 flex items-center gap-2">
                   <select value={genMode} onChange={(e) => setGenMode(e.target.value as GenerationMode)} className="text-xs p-1.5 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 focus:outline-none focus:border-orange-500">
                     <option value="auto">{t.mode.auto}</option>
                     <option value="standard">{t.mode.standard}</option>
                     <option value="html">{t.mode.html}</option>
                   </select>
                </div>
                <button onClick={handleGenerate} disabled={loading || (!prompt.trim() && !selectedImage)} className="absolute bottom-3 right-3 p-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 disabled:bg-gray-300 dark:disabled:bg-gray-600 transition-colors shadow-sm">
                  {loading ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} />}
                </button>
               </div>
            </div>
            <div className={`flex-1 flex flex-col min-h-0 ${isDarkMode ? 'bg-[#0d1117]' : 'bg-gray-50'}`}>
               {isMermaid && currentConfig ? (
                  <MermaidEditor key={currentId} config={currentConfig} onUpdate={handleConfigUpdate} lang={lang} isDarkMode={isDarkMode} />
               ) : isHtml && currentConfig ? (
                  <HtmlEditor key={currentId} config={currentConfig} onUpdate={handleConfigUpdate} lang={lang} isDarkMode={isDarkMode} />
               ) : (
                  <>
                    <div className={`px-4 py-2 text-xs font-mono border-b flex justify-between items-center ${isDarkMode ? 'bg-[#161b22] text-gray-400 border-gray-800' : 'bg-white text-gray-500 border-gray-200'}`}>
                        <div className="flex items-center gap-2"><CodeIcon size={14} /><span>{t.sourceCode}</span></div>
                        {codeError ? <span className="text-red-400 flex items-center gap-1"><AlertCircle size={12}/> {codeError}</span> : <span className="text-green-600 dark:text-green-500">{t.liveEditing}</span>}
                    </div>
                    <div className="flex-1 relative">
                       <CodeEditor value={editableCode} onChange={setEditableCode} language="json" isDarkMode={isDarkMode} />
                    </div>
                  </>
               )}
            </div>
          ...
          <div className="flex-1 bg-slate-100 dark:bg-black relative flex flex-col overflow-hidden transition-colors" style={{ width: `${100 - leftPanelWidth}%` }}>
             ...
             <div className="flex-1 p-4 lg:p-8 overflow-auto flex flex-col items-center min-h-0">
                {currentConfig ? (
                  <div 
                    className="w-full flex flex-col bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden relative group transition-all"
                    style={{ height: chartHeight }}
                  >
                      <div className="p-6 text-center border-b border-gray-50 dark:border-gray-700 bg-white dark:bg-gray-800">
                        <h2 className="text-2xl font-serif font-bold text-gray-900 dark:text-gray-100">{currentConfig.title}</h2>
                        {currentConfig.description && <p className="text-gray-500 dark:text-gray-400 mt-1 text-xs max-w-2xl mx-auto">{currentConfig.description}</p>}
                      </div>
                      <div className="flex-1 w-full min-h-0 p-4 bg-white dark:bg-gray-800 overflow-auto" id="chart-capture-target">
                         <ChartRenderer config={currentConfig} chartRef={chartContainerRef} isDarkMode={isDarkMode} palette={PALETTES[selectedPalette].colors} scale={zoomLevel} />
                      </div>
                      <div onMouseDown={handleMouseDown} className="absolute bottom-0 left-0 right-0 h-4 cursor-row-resize flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-gray-100 dark:hover:bg-gray-700 transition-opacity">
                         <div className="w-10 h-1 rounded-full bg-gray-300 dark:bg-gray-600" />
                      </div>
                  </div>
                ) : (
                  <div className="text-center text-gray-400 dark:text-gray-600 mt-20">
                    <div className="w-20 h-20 bg-gray-200 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-6"><Sparkles size={40} /></div>
                    <h3 className="text-xl font-medium">{t.preview}</h3>
                    <p className="max-w-xs mx-auto mt-2 text-sm">{t.previewHint}</p>
                  </div>
                )}
             </div>
          </div>
...
const App: React.FC = () => <ToastProvider><MainContent /></ToastProvider>;
export default App;